...
def _build_bom_runtime_instruction(
    ...
    return (
        ...
        "Never use an archived BOM, prior technical output, ./user_files/, memory, or another drawing as a replacement. "
        ...
        "Operate in BACKEND WORKFLOW MODE. Produce exactly one complete structured BOM JSON object; "
        "do not return a prose table and do not ask the user follow-up questions. "
        "Never invent a missing technical fact. Keep uncertainty inside status, certainty, assumptions, "
        "and points_to_confirm. "
        "For known Rod Choke material lines use the canonical component_id values ferrite_core, "
        "magnet_wire, lead_tinning, and glue when applicable. "
        "Every resolved numerical component consumption must have quantity_per_product and an explicit "
        "quantity_unit of pc, g, kg, m, or mm. Do not return an unlabelled numeric quantity. "
        "For magnet_wire, preserve developed wire length and mass as separate technical fields when they "
        "can be determined; never treat a length as a mass. "
        "Always make the lead_tinning decision explicit as confirmed, to_confirm, or not_required. "
        "If lead_tinning is explicitly not required, return its quantity as 0 g and explain the evidence. "
        "Do not calculate or impose supplier prices, commercial material costs, supplier identities, "
        "Incoterms, payment terms, delivered costs, or other External Component Costing values. "
        "Do not create manufacturing operations, MOST work packages, or process_scopes_for_most in the BOM; "
        "process decomposition is handled separately by the backend. "
        "After producing and validating the complete BOM JSON, call save_bom_output exactly once. "
        "Pass those exact project_code, product_id, and trigger_run_id values, and pass the complete BOM "
        "JSON object as raw_json. "
        ...
        "Treat write-back as successful only when the response confirms success=true, raw_bom_saved=true, "
        "normalized_bom_saved=true, and state.bom.status=received. Otherwise return BOM_WRITEBACK_FAILED."
    )
